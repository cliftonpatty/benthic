...
@onready var hand_open = preload("res://UI/Cursors/Asset 200.png")
@onready var hand_closed = preload("res://UI/Cursors/Asset 220.png")
...
func _physics_process(delta: float) -> void:
	...
	if drag_body:
		drag_body.position = lerp(drag_body.position, get_global_mouse_position(), 25 * delta)
	...
	if drag_body == null and recent_drag:
		...
		if detector.has_overlapping_areas():
			...
			nearestPos = Vector2(0,0)
		
		recent_drag.global_position = lerp(recent_drag.position, nearestPos, 25 * delta)
			
		if recent_drag.position.distance_to(nearestPos) < 0.002:
			print('Starting Settle')
			settle_grid(recent_drag)
			recent_drag = null

#Cycle through Grid points after tetro is dropped and decide what is covered
func settle_grid(body):
	
	for block in body.gridSettling.get_overlapping_areas():
		var foundBlock = $Grid.get_child($Grid.get_children().find(block))
		if foundBlock:
			foundBlock.blockCovered = true
			foundBlock.colChild.modulate = "#f9fa00"
			
	if len(body.gridSettling.get_overlapping_areas()) < body.gridCoverage : 
		print('ILLEGAL DROP')
...
func _on_meat_tetrominoes_dragged(body, dragged) -> void:
	if dragged:
		drag_body = body
	else:
		drag_body = null
